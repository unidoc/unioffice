...
func (m *CT_OMathArg) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	if m.EG_OMathElements != nil {
		for _, c := range m.EG_OMathElements {
	...
	for {
		...
		case xml.StartElement:
			...
			case xml.Name{Space: "http://schemas.openxmlformats.org/officeDocument/2006/math", Local: "acc"},
				...
				tmpomathelements := NewEG_OMathElements()
				...
				m.EG_OMathElements = append(m.EG_OMathElements, tmpomathelements)
				tmpomathelements.EG_OMathMathElements = append(tmpomathelements.EG_OMathMathElements, tmpomathmathelements)
...
func (m *CT_OMathArg) ValidateWithPath(path string) error {
	...
	for i, v := range m.EG_OMathElements {
		if err := v.ValidateWithPath(fmt.Sprintf("%s/EG_OMathElements[%d]", path, i)); err != nil {
			return err
		}
	}
	if m.CtrlPr != nil {
